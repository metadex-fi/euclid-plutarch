...
module Euclid.Plutarch.AssocMap (
  PMap (PMap),
...
pmergeWith = phoistAcyclic $
  plam $
    \combine -> punionWithData #$ plam $
      \x y -> pdata (combine # pfromData x # pfromData y)

data MapMergeConfig k v s = MapMergeConfig
  { ifNilXs :: Term s (PBuiltinListOfPairs k v :--> PBuiltinListOfPairs k v)
  , ifNilYs :: Term s (PBuiltinPair (PAsData k) (PAsData v) :--> PBuiltinListOfPairs k v :--> PBuiltinListOfPairs k v)
  , cont :: Term s (MergeInsert k v :--> )
  , delete :: Term s v
  }
  deriving stock (Generic)
  deriving anyclass (PlutusType)
instance DerivePlutusType (MapMergeConfig k v) where type DPTStrat _ = PlutusTypeScott

type MergeInsert k v = PBuiltinPair (PAsData k) (PAsData v) :--> PBuiltinListOfPairs k v :--> PBuiltinListOfPairs k v :--> PBuiltinListOfPairs k v
...
data MapMergeCarrier k v s = MapMergeCarrier
  ...
  , mergeInsert :: Term s (MergeInsert k v)
  }
  deriving stock (Generic)
  deriving anyclass (PlutusType)
...
-- TODO WIP, also overengineering for now. don't
mapMergeCarrier :: (POrd k, PIsData k) => Term s (MapMergeConfig :--> (PAsData v :--> PAsData v :--> PAsData v) :--> MapMergeCarrier k v :--> MapMergeCarrier k v)
mapMergeCarrier = phoistAcyclic $ plam \combine self ->
...
mapMerge :: forall k v s. (POrd k, PIsData k) => Term s ((PAsData v :--> PAsData v :--> PAsData v) :--> MapMergeCarrier k v)
mapMerge = phoistAcyclic $ plam \combine -> punsafeCoerce pfix # (mapMergeCarrier # combine :: Term _ (MapMergeCarrier k v :--> MapMergeCarrier k v))
...
pmergeWithData = phoistAcyclic $
  plam $ \combine x y ->
    pcon $ PMap $ (pmatch (mapMerge # combine) \(MapMergeCarrier {merge}) -> merge) # pto x # pto y
