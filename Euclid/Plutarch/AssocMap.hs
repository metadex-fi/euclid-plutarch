...
-- {- | Merge two 'PMap's applying the given function to any two values that
--  share the same key. Error if keys don't align. Remove null-values.
-- -}
-- parithWith ::
--   (POrd k, PIsData k, PIsData v) =>
--   Term s ((v :--> v :--> v) :--> PMap 'Sorted k v :--> PMap 'Sorted k v :--> PMap 'Sorted k v)
-- parithWith = phoistAcyclic $
--   plam $
--     \combine -> parithWithData #$ plam $
--       \x y -> pdata (combine # pfromData x # pfromData y)

-- mapArithCarrier :: (POrd k, PIsData k) => Term s ((PAsData v :--> PAsData v :--> PAsData v) :--> MapMergeCarrier k v :--> MapMergeCarrier k v)
-- mapArithCarrier = phoistAcyclic $ plam \combine self ->
--   let mergeInsert = pmatch self \(MapMergeCarrier {mergeInsert}) -> mergeInsert
--       merge = pmatch self \(MapMergeCarrier {merge}) -> merge
--    in pcon $
--         MapMergeCarrier
--           { merge = plam $ \xs ys -> pmatch xs $ \case
--               PNil -> perror -- diff to mapUnionCarrier here
--               PCons x xs' -> mergeInsert # x # xs' # ys
--           , mergeInsert = plam $ \x xs ys ->
--               pmatch ys $ \case
--                 PNil -> perror -- diff to mapUnionCarrier here
...
-- mapArith :: forall k v s. (POrd k, PIsData k) => Term s ((PAsData v :--> PAsData v :--> PAsData v) :--> MapMergeCarrier k v)
-- mapArith = phoistAcyclic $ plam \combine -> punsafeCoerce pfix # (mapArithCarrier # combine :: Term _ (MapMergeCarrier k v :--> MapMergeCarrier k v))

-- {- | Merge two 'PMap's applying the given function to any two data-encoded
--  values that share the same key. Error if keys don't align. Remove null-values.
-- -}
-- parithWithData ::
--   (POrd k, PIsData k) =>
--   Term
...
--         :--> PMap 'Sorted k v
--     )
-- parithWithData = phoistAcyclic $
--   plam $ \combine x y ->
--     pcon $ PMap $ (pmatch (mapArith # combine) \(MapMergeCarrier {merge}) -> merge) # pto x # pto y
