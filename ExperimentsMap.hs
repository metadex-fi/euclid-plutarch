...
import Euclid.Plutarch.AssocMap
  ...
  let info = pfield @"txInfo" # ctx
      ...
      cond = plam $ \ref ->
        ...
        in pmatch dat $ \case
          ...
          _ -> pconstant False
  in pif 
      (Plutarch.Prelude.pany # cond # refs)
      (popaque $ pcon PUnit)
      perror
...
ptestInsec :: Term s PString
ptestInsec = P.do
  let one = pconstant @PInteger 1
      two = pconstant @PInteger 2
      three = pconstant @PInteger 3
      four = pconstant @PInteger 4
      five = pconstant @PInteger 5

    
      m0 = pempty
      m1 = pinsert # one # two # m0
      m2 = pinsert # two # three # m0
      m3 = pinsert # three # five # m1

      mr = pinsecWith # (plam (+)) # m3 # m1
  pshow mr